...
class Modoboa(base.Installer):
    ...
    def _setup_venv(self):
        ...
        if self.devmode:
            ...
            python.install_packages(
                ["django-bower", "django-debug-toolbar"],
                self.venv_path,
                upgrade=self.upgrade,
                sudo_user=self.user,
            )
    ...
    def _deploy_instance(self):
        ...
        if os.path.exists(target):
            ...
            if condition:
                utils.printcolor(
                    ...
                    "exists. If you choose to continue, it will be removed.".format(
                        target
                    ),
                    utils.YELLOW,
                )
        ...
        prefix = ". {}; ".format(os.path.join(self.venv_path, "bin", "activate"))
        args = [
            "--collectstatic",
            "--timezone",
    ...
    def setup_database(self):
        ...
        if not self.amavis_enabled:
            return
        self.backend.grant_access(self.config.get("amavis", "dbname"), self.dbuser)
    ...
    def apply_settings(self):
        ...
        webmail_media_dir = os.path.join(self.instance_path, "media", "webmail")
        pw = pwd.getpwnam(self.user)
        for d in [rrd_root_dir, pdf_storage_dir, webmail_media_dir]:
            utils.mkdir(d, stat.S_IRWXU | stat.S_IRWXG, pw[2], pw[3])
        settings = {
            "admin": {"handle_mailboxes": True, "account_auto_removal": True},
            "modoboa_amavis": {
                "am_pdp_mode": "inet",
            },
            "maillog": {
        ...
        if self.config.getboolean("opendkim", "enabled"):
            settings["admin"]["dkim_keys_storage_dir"] = self.config.get(
                "opendkim", "keys_storage_dir"
            )
        settings = json.dumps(settings)
        query = "UPDATE core_localconfig SET _parameters='{}'".format(settings)
        self.backend._exec_query(query, self.dbname, self.dbuser, self.dbpasswd)
